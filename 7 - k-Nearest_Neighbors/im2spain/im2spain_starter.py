...
def plot_3nn(test_image_filename, nn_filenames):
    ...
    # Plot test image
    axes[0].imshow(imageio.imread(f'im2spain/im2spain_images/{test_image_filename}'))
    axes[0].set_title('test image')
    axes[0].axis('off')

    # Plot nearest neighbors
    for i in range(3):
        axes[i+1].imshow(imageio.imread(f'im2spain/im2spain_images/{test_image_filename}'))
...
def displacement_error(y, y_hat):
    ...
    Returns:
        Displacement error
    """
    lat_error = (y[:, 0] - y_hat[:, 0]) * 69
    lon_error = (y[:, 1] - y_hat[:, 1]) * 52
    disp_error = np.sqrt(lat_error**2 + lon_error**2)
    return disp_error
    ...
    for k in ks:
        ...
        for i, nearest in enumerate(indices):
            ...
            if is_weighted:
                ...
                y_hat = np.average(train_labels[nearest], axis=0, weights=weights)
            else:
...
def main():
    ...
    test_index = list(test_files).index(test_image_filename)
    test_feature = test_features[test_index].reshape(1, -1)
    
    k_nearest_neighbor = NearestNeighbors(n_neighbors=3).fit(train_features)
    distances, indexes = k_nearest_neighbor.kneighbors(test_feature)
    
    nn_coords = train_labels[indexes[0]]
    nn_filenames = train_files[indexes[0]]
    
    print("Test image:", test_image_filename)
    print("Nearest neighbor filenames:", nn_filenames)
    print("Coordinates of 3-NN:")
    for i, coord in enumerate(nn_coords):
        print(f"Neighbor {i+1}: Lat {coord[0]:.4f}, Lon {coord[1]:.4f}")
    ...
    mean_coord = np.mean(train_labels, axis=0)
    preds = np.tile(mean_coord, (test_labels.shape[0], 1))
    
    naive_error = np.mean(displacement_error(test_labels, preds))
    ...
    for r in ratios:
        ...
        subset_X = train_features[:num_samples]
        subset_y = train_labels[:num_samples]
        
        #linear regression
        lin_reg = LinearRegression().fit(subset_X, subset_y)
        y_hat_lin = lin_reg.predict(test_features)

        e_lin = np.mean(displacement_error(test_labels, y_hat_lin))
        e_nn = grid_search(subset_X, subset_y, test_features, test_labels, verbose=False)
